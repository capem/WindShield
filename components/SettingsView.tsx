...
const ToggleSwitch: React.FC<{ checked: boolean; onChange: (checked: boolean) => void; id: string; }> = ({ checked, onChange, id }) => {
    return (
        <div className="relative inline-block w-10 align-middle select-none transition duration-200 ease-in">
            ...
            <label
                htmlFor={id}
                className={`block overflow-hidden h-6 rounded-full cursor-pointer transition-colors ${checked ? 'bg-violet-500' : 'bg-gray-300 dark:bg-gray-600'}`}
            >
            </label>
...
const CardSizePreview: React.FC<{
    ...
    <div className="text-center w-40">
        <button
            onClick={onClick}
            className={`rounded-lg p-3 border-2 transition-all duration-200 w-full h-36 flex flex-col items-center justify-center shadow-sm ${
                isSelected
                    ? 'border-violet-500 bg-violet-50 dark:bg-violet-900/50 scale-105'
                    : 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700/50 hover:border-violet-400 dark:hover:border-violet-600'
            }`}
        >
            {children}
        </button>
        <p className={`mt-2 text-sm font-semibold transition-colors ${isSelected ? 'text-violet-600 dark:text-violet-400' : 'text-gray-600 dark:text-gray-400'}`}>
            {title}
        </p>
    </div>
);
    ...
    return (
        <div className="animate-fade-in max-w-4xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 dark:bg-gray-800">
                <h2 className="text-xl font-bold text-gray-800 mb-2 pb-3 border-b-2 border-violet-200 dark:text-gray-100 dark:border-violet-700">Appearance</h2>
                
                <SettingRow title="Theme" description="Switch between light and dark visual modes.">
                    <div className="flex items-center gap-2">
                        <button onClick={() => setIsDarkMode(false)} className={`px-4 py-2 rounded-lg font-semibold transition-colors text-sm ${!isDarkMode ? 'bg-violet-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600'}`}>
                            <i className="fa-solid fa-sun mr-2"></i> Light
                        </button>
                        <button onClick={() => setIsDarkMode(true)} className={`px-4 py-2 rounded-lg font-semibold transition-colors text-sm ${isDarkMode ? 'bg-violet-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600'}`}>
                ...
                <div className="py-4 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h4 className="font-semibold text-gray-800 dark:text-gray-100">Turbine Card Size</h4>
                    ...
                    <div className="flex justify-center items-start gap-8 mt-2">
                        <CardSizePreview title="Expanded" isSelected={!isCompactView} onClick={() => setIsCompactView(false)}>
                            <div className="w-full text-center space-y-2 bg-white dark:bg-gray-800 p-2 rounded-md">
                        ...
                        <CardSizePreview title="Compact" isSelected={isCompactView} onClick={() => setIsCompactView(true)}>
                             <div className="w-full h-full text-left bg-white dark:bg-gray-800 p-2 rounded-md flex flex-col justify-between">
                                <p className="text-xs font-bold text-gray-700 dark:text-gray-300">T 001</p>
