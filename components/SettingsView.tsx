...
const ToggleSwitch: React.FC<{
	...
	return (
		<div className="relative inline-block w-10 align-middle select-none transition duration-200 ease-in">
			...
			<span
				className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform duration-300 ease-in-out ${checked ? "translate-x-4" : ""}`}
			></span>
		</div>
	);
};
...
const CardSizePreview: React.FC<{
	...
	<div className="text-center w-40">
		...
		<p
			className={`mt-2 text-sm font-semibold transition-colors transition-theme ${isSelected ? "text-violet-600 dark:text-violet-400" : "text-gray-600 dark:text-gray-400"}`}
		>
			{title}
		</p>
	</div>
...
const SettingsView: React.FC<SettingsViewProps> = ({
	...
	return (
		<div className="animate-fade-in max-w-4xl mx-auto">
			<h1 className="text-3xl font-bold text-gray-900 mb-6 dark:text-white transition-theme">
				Settings
			</h1>
			<div className="bg-white rounded-xl shadow-sm p-6 dark:bg-black transition-theme">
				<h2 className="text-xl font-bold text-gray-800 mb-2 pb-3 border-b-2 border-violet-200 dark:text-white dark:border-violet-700 transition-theme">
					Appearance
				</h2>
				...
				>
					<div className="flex items-center gap-2">
						<button
							...
							className={`px-4 py-2 rounded-lg font-semibold transition-colors text-sm transition-theme ${!isDarkMode ? "bg-violet-500 text-white" : "bg-gray-800 text-gray-200 hover:bg-gray-700"}`}
						>
