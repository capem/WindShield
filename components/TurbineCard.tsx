...
const TurbineCard: React.FC<TurbineCardProps> = ({ turbine, onClick, isCompact = false, activeAlarmSeverity = null }) => {
    const statusConfig = {
        [TurbineStatus.Producing]: { text: 'Producing', classes: 'text-green-700 dark:text-green-300 bg-green-100 dark:bg-green-900/30 border-green-500' },
            ...
            <button type="button" onClick={onClick} className={`bg-white dark:bg-black rounded-lg p-2 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 ${borderClass} border-l-4 flex flex-col justify-between text-left w-full h-full transition-theme-fast`}>
                <div className="flex justify-between items-center mb-1">
                    <div className="flex items-center gap-1.5">
                        ...
                        <h3 className="font-bold text-slate-800 dark:text-white text-xs">{turbine.id}</h3>
                    </div>
                ...
                <div className="flex items-center justify-around gap-2 mt-1 flex-grow">
                    ...
                    <div className="text-xs text-center space-y-0.5">
                        <div>
                            <p className="text-slate-500 dark:text-gray-400 text-[10px]">Active Power</p>
                            <p className="font-semibold text-slate-900 dark:text-white text-xs">
                                {turbine.activePower !== null ? `${turbine.activePower.toFixed(1)} MW` : '—'}
                            </p>
                        </div>
                        <div>
                            <p className="text-slate-500 dark:text-gray-400 text-[10px]">Wind Speed</p>
                            <p className="font-semibold text-slate-900 dark:text-white text-xs">
            ...
            <div>
                <div className="flex justify-between items-center mb-2">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold text-slate-800 dark:text-white text-sm">{turbine.id}</h3>
                    </div>
