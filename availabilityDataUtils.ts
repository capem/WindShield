...
export const generateAlarmData = (turbineId: string): AvailabilityAlarm[] => {
	...
	for (let i = 0; i < alarmCount; i++) {
		const alarmDef =
			ALARM_DEFINITIONS[Math.floor(Math.random() * ALARM_DEFINITIONS.length)];
		...
		const timeOff = isActive
			? null
			: new Date(timeOn.getTime() + Math.random() * 24 * 60 * 60 * 1000);

		const duration = timeOff
			? (timeOff.getTime() - timeOn.getTime()) / (1000 * 60 * 60)
			: (now.getTime() - timeOn.getTime()) / (1000 * 60 * 60);
...
export const generateTimestampData = (): TimestampData[] => {
	...
	for (
		let time = new Date(sevenDaysAgo);
		time <= now;
		...
		const activeAlarms = hasActiveAlarms
			? [
					ALARM_DEFINITIONS[
						Math.floor(Math.random() * ALARM_DEFINITIONS.length)
					].name,
				]
			: [];
...
export const calculateEnergyLoss = (
	params: EnergyLossCalculationParams,
): {
	...
	if (isExcusable) {
		return {
			nonExcusableEnergyLost: 0,
			excusableEnergyLost: potentialEnergyKWh,
		};
	} else {
		return {
			nonExcusableEnergyLost: potentialEnergyKWh,
			excusableEnergyLost: 0,
		};
	}
};
...
export const calculateSummaryStats = (
	...
	const totalNonExcusableLoss = alarmData.reduce(
		(sum, alarm) => sum + alarm.nonExcusableEnergyLost,
		0,
	);
	const totalExcusableLoss = alarmData.reduce(
		(sum, alarm) => sum + alarm.excusableEnergyLost,
		0,
	);
	const totalLoss = totalNonExcusableLoss + totalExcusableLoss;

	const totalUndefinedLoss = timestampData.reduce(
		(sum, item) => sum + item.energyLostUndefined,
		0,
	);

	const totalEnergyProduced = timestampData.reduce(
		(sum, item) => sum + item.energyProduced,
		0,
	);
	...
	return {
		...
		availabilityPercentage:
			totalEnergyProduced > 0
				? (
						(totalEnergyProduced / (totalEnergyProduced + totalLoss)) *
						100
					).toFixed(2)
				: "0.00",
	};
